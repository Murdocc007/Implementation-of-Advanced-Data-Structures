...
public class LP3_Level2 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		// check for negative cycles, if negative cycle exists then return the negative cycle
		...
		if(negativeCycle){
			...
			// if no negative cycle exists then find all the shortest paths for each vertex
			level2(graph);
			printResult(graph);
	...
	/*
	 * Function to find the different shortest paths in a graph
	 * and printing the summation of all the shortest paths in the graph
	 * 
	 * */
	public static void level2(Graph g){
		...
		// initialize the source in the topologically ordered list with the source vertex
		int i;
		 ...
		 * 
		 * Case 1) If the vertex is not seen, set the distance as the distance till parent and the weight of the node
		 * Case 2) If the vertex id already seen then
		 * 			i) If distance from current node is equal to its distance already obtained
		 *             then that means, another shortest path was discovered for this node
		 *             hence, increase the spCount of the current node with the spCount of the parent node
		 *         ii) If the distance from current node is less than the existing distance, update the distance
	...
	/*
	 * Function to detect negative cycle in a graph
	 * If negative cycle exists return true else return false
	 * 
